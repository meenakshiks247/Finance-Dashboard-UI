...
export function Header() {
  ...
  return (
    <header className="navbar-shell sticky top-0 z-20 border-b backdrop-blur-lg">
      <div className="mx-auto max-w-7xl px-4 py-2.5 lg:px-8">
        <div className="theme-surface flex flex-col gap-2.5 rounded-[1.75rem] border px-4 py-2.5 transition-colors md:px-5 lg:grid lg:grid-cols-[auto_minmax(0,1fr)_auto] lg:items-center lg:gap-4">
          ...
          <nav className="flex gap-2 overflow-x-auto pb-0.5 lg:justify-center lg:pb-0">
            {navigationItems.map((item) => (
              <a
                key={item.label}
                href={item.href}
                aria-current={item.active ? 'page' : undefined}
                className={`motion-tab inline-flex h-8 items-center whitespace-nowrap rounded-full px-3 text-xs font-semibold sm:text-sm ${
                  item.active
          ...
          <div className="grid gap-2 sm:grid-cols-2 sm:items-center lg:flex lg:flex-wrap lg:justify-end">
            <label className="motion-control theme-surface-soft flex h-9 items-center gap-2.5 rounded-2xl border px-3 text-sm shadow-sm">
            ...
            <button
              type="button"
              onClick={toggleTheme}
              className="btn btn-secondary motion-control h-9 px-3.5"
          ...
          <div className="theme-muted flex items-center justify-between gap-3 border-t theme-border pt-2 text-xs sm:text-sm lg:col-span-3">
